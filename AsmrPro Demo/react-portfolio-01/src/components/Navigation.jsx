...
const Navigation = () => {
  ...
  return (
    <nav
      className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? "bg-white/95" : "bg-transparent"}`}
    >
          ...
          <div className="bg-white border border-gray-100 rounded-lg shadow-lg p-4 space-y-4">
            {navItems.map((item) => (
              <a
                ...
                className="block text-gray-600 hover:text-black border-gray-100 rounded-lg p-2"
              >
                {item.label}
              </a>
            ))}
