...
const Navigation = () => {
    ...
    const [isScrolled, setIsScrollTop] = useState(false);
    ...
    useEffect(() => {
        const handleScroll = () => {
            setIsScrollTop(window.scrollY > 50);
        }
    ...
    return (
        <nav className= {`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white/95' : 'bg-transparent'}`}>
            <div className="max-w-6xl mx-auto px-6 py-4">
                <div className="flex justify-between items-center">
                    ...
                    <div className="hidden md:flex space-x-8">
                        {navItems.map((item) => {
                            return (
                                <a key={item.href} href={item.href} className={`transition-colors ${isScrolled ? `text-gray-600 hover:text-black` : `text-gray-700 hover:text-black`}`
                                } onClick={(e) => {
                                    e.preventDefault();
